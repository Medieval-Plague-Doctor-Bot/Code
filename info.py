...
class info(commands.Cog):
  ...
  async def serverinfo(self, ctx):
      ...
      em.add_field(name="Channels",value=f"{len(ctx.guild.channels)} total channels\n{len(ctx.guild.categories)} total categories\n{len(ctx.guild.text_channels)} text channels\n{len(ctx.guild.voice_channels)} voice channels")
  ...
  async def userinfo(self, ctx, member:discord.Member = None):
      ...
      all_members = [mem.id for mem in all_members]
      joinpos = all_members.index(member.id)+1
      em.add_field(name="Account creation position",value=joinpos)
  ...
  async def botinfo(self,ctx):
    ...
    em.add_field(name="creator",value="!!!ahhhhh!!!#7441")
    await ctx.send(embed=em)
